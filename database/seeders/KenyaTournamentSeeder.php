...
class KenyaTournamentSeeder extends Seeder
{
    ...
    private function createUsers($communities): array
    {
        $users = [];
        ...
        foreach ($communities as $communityId) {
            $community = DB::table('communities')->find($communityId);
            $numUsers = rand(12, 20); // 12-20 users per community
            
            for ($i = 1; $i <= $numUsers; $i++) {
                $username = 'player_c' . $communityId . '_u' . $i;
                $preferredDays = $preferredDaysOptions[array_rand($preferredDaysOptions)];
                
                // Determine player level based on random distribution
                $levelRand = rand(1, 100);
                if ($levelRand <= 5) {
                    $level = 'national';
                    $points = rand(2000, 3000);
                } elseif ($levelRand <= 15) {
                    $level = 'regional';
                    $points = rand(1500, 2000);
                } elseif ($levelRand <= 40) {
                    $level = 'county';
                    $points = rand(1000, 1500);
                } else {
                    $level = 'community';
        ...
        $users[] = DB::table('users')->insertGetId([
            ...
            'county_id' => DB::table('communities')->find($communities[0])->county_id,
            'region_id' => DB::table('communities')->find($communities[0])->region_id,
            'total_points' => 5000,
            'email_verified_at' => Carbon::now(),
            'created_at' => Carbon::now(),
            'updated_at' => Carbon::now(),
